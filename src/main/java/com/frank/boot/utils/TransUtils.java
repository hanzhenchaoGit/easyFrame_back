...
public class TransUtils {
     ...
     *            待转化类
     * @param format自定义转化类型
     * @return Map<String,Object> 将传入的DTO转化为Map，1、Date转化为String类型的时间戳。 2、null转化为null
     *         3、enum通过反射使用getCode()获取code编码。4、其他类型不做转换value=toString()。5、Dto中属性名中的大写字母将被替换为
     *         "_小写字母"形式，为了与库中返回map一致。
     */
    public static Map<String, Object> objectToMap(Object object, FormatField format) {
        if (object == null) {
            return null;
        }
